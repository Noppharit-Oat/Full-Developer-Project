...
class ChecklistService {
    ...
    async getAllChecklistGroups() {
        const result = await pool.query('SELECT * FROM checklist_groups ORDER BY id');
        return result.rows;
    }

    async getChecklistGroupById(id) {
        const result = await pool.query('SELECT * FROM checklist_groups WHERE id = $1', [id]);
    ...
    async deleteChecklistGroup(id) {
        const result = await pool.query('DELETE FROM checklist_groups WHERE id = $1 RETURNING *', [id]);
        return result.rows[0];
    }
